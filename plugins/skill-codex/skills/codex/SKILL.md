...
## Running a Task
1. Ask the user (via `AskUserQuestion`) which model to run (`gpt-5.3-codex` or `gpt-5.2`) AND which reasoning effort to use (`xhigh`, `high`, `medium`, or `low`) in a **single prompt with two questions**, if user prompt contains codex [default] then use gpt-5.3-codex and high, no need ask.
